...
@pytest.fixture
async def client(pages: PageStore) -> AsyncIterator[httpx.AsyncClient]:
...
async def test_get_page_is_public_and_learns_base_url(client: httpx.AsyncClient, pages: PageStore) -> None:
    assert pages.base_url == ""
    assert (await client.get("/health")).status_code == 200
    assert pages.base_url == "http://brain.test", "first request fixes the base URL when DAYFLOW_PUBLIC_URL is unset"
    page = await pages.put("report", "<html><body>Lab 1 report</body></html>")
    assert page["url"].startswith("http://brain.test/pages/report/")
