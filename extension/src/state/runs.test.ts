...
describe('applyEvent', () => {
  it('merges streamed partial text into one step', () => {
    let run = newRun('r1', 's1', 'x', undefined, 0);
  ...
  it('keeps the full prompt and counts browser actions', () => {
    let run = newRun('r1', 's1', 'Sync my files from WSP into the vault, please', 'vault-sync', 0);
    expect(run.prompt).toBe('Sync my files from WSP into the vault, please');
    expect(run.sessionId).toBe('s1'); // the conversation, not the run id
    run = applyEvent(run, { kind: 'tool.call', target: 'browser', call: { id: 'c1', name: 'read_page', args: {} } });
  ...
  it('handles pause and resume transitions', () => {
    ...
    run = applyEvent(run, { kind: 'run.pause' });
    expect(run.status).toBe('paused');
    run = applyEvent(run, { kind: 'run.resume' });
    expect(run.status).toBe('running');
  });

  it('does not reopen a terminal run with late events', () => {
    let run = newRun('r1', 's1', 'x', undefined, 0);
    run = applyEvent(run, { kind: 'run.end', status: 'cancelled', summary: 'Cancelled' });
    run = applyEvent(run, { kind: 'text', text: 'late chunk' });
    run = applyEvent(run, { kind: 'run.resume' });
    run = applyEvent(run, { kind: 'run.end', status: 'done', summary: 'Done' });
    expect(run.status).toBe('cancelled');
    expect(run.summary).toBe('Cancelled');
